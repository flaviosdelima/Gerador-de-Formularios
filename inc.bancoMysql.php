...
<?php
	...
	$connexao=mysql_connect($_SESSION['server_banco'],$_SESSION['usu_banco'],$_SESSION['usu_senha'])or die("Erro na conexao".mysql_error());
...
if(isset($_POST['cria_codigo']))
{
 $campo="Exemplo Campos\n<!--\n <form name=\"\" method=\"post\">";
	foreach($_POST as $linha=>$valor)
	{
		if($linha=="cria_codigo")break; //se chegar ao fim deve sair do for
		 ...
		 switch($valor)
		 {
		  case "nada":
			...
			break;
		  case "textarea": 
			$campo.= "<label>".$linha."</label>\n";
			$campo.= "<!--textarea name='txta_".$linha."'>teste</textarea-->";
			break;
		case "file": 
			$campo.= "<label>".$linha."</label>\n";
			$campo.= "<input type='".$valor."' name='fl".$linha."'/>\n";
			break;
		  case "select":
			$campo.= "<label>".$linha."</label>\n";
	  ...
	  $campo.="<input type=\"submit\" name=\"btn_submit\" value=\"Cadastrar\" /> </form>\n-->";
